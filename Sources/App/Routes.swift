...
final class Routes: RouteCollection {
    ...
    func build(_ builder: RouteBuilder) throws {
        ...
        builder.get("retrieve", "statistics", "administrator", "dashboard", "primary", ":id") { request in
            ...
            let users = try User.makeQuery().filter("id", .notEquals, id).all()
        ...
        builder.post("create", "user", "requestor", ":requestorId") { request in
            ...
            let neKeys = ["lastName", "firstName", "gender", "type", "username", "encryptedUsername", "password", "encryptedPassword", "owner"]
            ...
            if values != nil {
                ...
                if users.count > 0 {
                    ...
                    self.status = 0
                    self.message = "A new user has been successfully created."
                    self.data = ["user": user]
                    
                    try self.logActivity("Created a new user with id \"\(user.id?.int ?? 0)\"", inModule: "User", byRequest: request)
                }
            }
            else {
                self.status = 1
        ...
        builder.post("create", "clue", "requestor", ":requestorId") { request in
            ...
            if values != nil {
                ...
                if let choiceData = choiceString.data(using: .utf8, allowLossyConversion: true) {
                    if let choiceList = self.json(fromData: choiceData) as? [[String: Any]], choiceList.count > 0 {
                        ...
                        if let newClue = try Clue.makeQuery().all().last {
                            ...
                            if didRelateChoices {
                                self.status = 0
                                self.message = "A new clue has been successfully created."
                                self.data = ["clue": newClue]
                                try self.logActivity("Created clue with id \"\(newClue.id?.int ?? 0)\"", inModule: "Clue", byRequest: request)
                            }
                        }
                    }
                }
                ...
                if !didRelateChoices {
                    self.status = 1
                    self.message = "Creating a new clue failed!"
                    self.data = nil
                }
            }
            else {
        ...
        builder.get("retrieve", "clues", "user", ":id", "requestor", ":requestorId") { request in
            ...
            try self.logActivity("Retrieved clues created by user with id \"\(id)\"", inModule: "Clue", byRequest: request)
            ...
            if values != nil {
                ...
                if let choiceData = choiceString.data(using: .utf8, allowLossyConversion: true) {
                    if let choiceList = self.json(fromData: choiceData) as? [[String: Any]], choiceList.count > 0 {
                        ...
                        if didRelateChoices {
                            self.status = 0
                            self.message = "Clue has been successfully updated."
                            self.data = ["clue": newClue]
                            try self.logActivity("Updated clue with id \"\(newClue.id?.int ?? 0)\"", inModule: "Clue", byRequest: request)
                        }
                    }
                }
                ...
                if !didRelateChoices {
                    self.status = 1
                    self.message = "Updating clue with id \"\(clue.id?.int ?? 0)\" failed!"
                    self.data = nil
                }
            }
            else {
        ...
        builder.post("delete", "clue", ":id", "requestor", ":requestorId") { request in
            ...
            self.status = 0
            self.message = "Clue with id \"\(clue.id?.int ?? 0)\" has been successfully deleted."
            self.data = ["clue": clue]
            
            try self.logActivity("Deleted clue with id \"\(clue.id?.int ?? 0)\"", inModule: "Clue", byRequest: request)
        ...
        builder.post("create", "treasure", "requestor", ":requestorId") { request in
            ...
            if values != nil {
                ...
                self.status = 0
                self.message = "A new treasure has been successfully created."
                self.data = ["treasure": treasure]
                
                try self.logActivity("Created a new treasure with id \"\(treasure.id?.int ?? 0)\"", inModule: "Treasure", byRequest: request)
            }
            else {
                self.status = 1
                self.message = "Sent wrong data!"
        ...
        builder.get("retrieve", "treasures", "user", ":id", "requestor", ":requestorId") { request in
            guard let id = request.parameters["id"]?.int else { throw Abort.badRequest }
            let treasures = try Treasure.makeQuery().filter("owner", .equals, id).all()
            try self.logActivity("Retrieved treasures created by user with id \"\(id)\"", inModule: "Treasure", byRequest: request)
        ...
        builder.post("update", "treasure", ":id", "requestor", ":requestorId") { request in
            ...
            if values != nil {
                ...
                self.status = 0
                self.message = "Treasure with id \"\(treasure.id?.int ?? 0)\" has been successfully updated."
                self.data = ["treasure": treasure]
                
                try self.logActivity("Updated treasure with id \"\(id)\"", inModule: "Treasure", byRequest: request)
            }
            else {
                self.status = 1
                self.message = "Sent wrong data!"
        ...
        builder.post("delete", "treasure", ":id", "requestor", ":requestorId") { request in
            ...
            self.status = 0
            self.message = "Treasure with id \"\(treasure.id?.int ?? 0)\" has been successfully deleted."
            self.data = ["treasure": treasure]
            
            try self.logActivity("Deleted treasure with id \"\(treasure.id?.int ?? 0)\"", inModule: "Treasure", byRequest: request)
        ...
        builder.post("create", "game", "requestor", ":requestorId") { request in
            ...
            if values != nil {
                ...
                self.status = 0
                self.message = "A new game has been successfully created."
                self.data = ["game": game]
                
                try self.logActivity("Created a new game with id \"\(game.id?.int ?? 0)\"", inModule: "Game", byRequest: request)
            }
            else {
                self.status = 1
                self.message = "Sent wrong data!"
        ...
        builder.get("retrieve", "games", "user", ":id", "requestor", ":requestorId") { request in
            ...
            try self.logActivity("Retrieved games created by user with id \"\(id)\"", inModule: "Game", byRequest: request)
        ...
        builder.post("update", "game", ":id", "requestor", ":requestorId") { request in
            ...
            if values != nil {
                ...
                self.status = 0
                self.message = "Game with id \"\(game.id?.int ?? 0)\" has been successfully updated."
                self.data = ["game": game]
                
                try self.logActivity("Updated game with id \"\(game.id?.int ?? 0)\"", inModule: "Game", byRequest: request)
            }
            else {
                self.status = 1
                self.message = "Sent wrong data!"
        ...
        builder.post("delete", "game", ":id", "requestor", ":requestorId") { request in
            ...
            self.status = 0
            self.message = "Game with id \"\(game.id?.int ?? 0)\" has been successfully deleted."
            self.data = ["game": game]
            
            try self.logActivity("Deleted game with id \"\(game.id?.int ?? 0)\"", inModule: "Game", byRequest: request)
        ...
        builder.post("deploy", "game", ":gameId", "requestor", ":requestorId") { request in
            ...
            if values != nil {
                ...
                self.status = 0
                self.message = "Game with id \"\(gameId)\" has been successfully deployed!"
                self.data = nil
                
                try self.logActivity("Deployed game with id \"\(gameId)\" to \(deployedToClasses.count > 1 ? "classes" : "class") with \(deployedToClasses.count > 1 ? "ids" : "id") \(deployedToClasses)", inModule: "Game", byRequest: request)
            }
            else {
                self.status = 1
                self.message = "Sent wrong data!"
            ...
            if let game = try Game.makeQuery().filter("id", .equals, gameId).all().last {
                ...
                for clue in clues {
                    if !clueIds.contains(clue.id?.int ?? 0) {
                        ...
                        clueNames = "\(clueNames == "" ? "" : "\(clueNames)")<td nowrap><font size=\"100\">\(clue.clue)</br>(PTS/ATT)</font></td>"
                    }
                }
                ...
                if let klase = try Class.makeQuery().filter("id", .equals, classId).all().last {
                    ...
                    let htContent = "<tr bgcolor=\"green\"><td nowrap><font size=\"100\">Players</font></td>\(clueNames)\(treasureName)<td nowrap><font size=\"100\">Total Points</font></td><td nowrap><font size=\"100\">Date Played</font></td><td nowrap><font size=\"100\">Rank</font></td></tr>"
                    ...
                    let ftContent = "<tr bgcolor=\"yellow\"><td nowrap><font size=\"100\"><b>Average</b></font></td>\(acaContent)\(ataContent)<td nowrap><font size=\"100\"><b>\(accTotalPointsString)</b></font></td><td nowrap><font size=\"100\"><b>NA</b></font></td><td nowrap><font size=\"100\"><b>NA</b></font></td></tr>"
